...
import {SERCET_KEY} from "../middleware/auth"
class AuthController {
    ...
    login = async (req: Request,res: Response) => {
        ...
        if(!user){
            ...
            let comparePassword = await bcrypt.compare(loginForm.passwourd,user.passwourd);
            if(!comparePassword){
                res.status(401).json({
                ...
                let token = await jwt.sign(payload,SERCET_KEY,{
                    expiresIn: 360000
                })
                res.status(201).json({
                    token:token
